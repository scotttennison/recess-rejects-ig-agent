...
async function generateImage(brand, imagePrompt) {
  const fullPrompt = `Image 1 attached is the official, exact mascot character design for this brand — a red kickball character. Its tongue is shaped exactly like a human foot sticking out of its mouth: it has a distinct heel, an arch, and five small rounded toes at the end, not a normal flat tongue. This foot-shaped tongue is the single most important and unusual identifying feature of the mascot — do not simplify it into a regular tongue shape. Copy the mascot's face, this exact foot-shaped tongue, and its color exactly as shown in image 1, unchanged. Any ball that appears anywhere in the scene, whether being held, kicked, or thrown, or just sitting as a prop in the background, must be the same red rubber kickball as the mascot itself — never a basketball, soccer ball, or any other ball type. Only change the pose, body position, and surrounding scene to fit this new context: ${imagePrompt}. Image 2 attached is the brand's official wordmark showing the exact font style, letterforms, and drop-shadow effect to use for text in this illustration — match this bold condensed athletic lettering style AND its exact rust/burnt-orange text color with black drop-shadow exactly. If any headline or title text appears in the image, it must be the brand name "RECESS REJECTS" spelled exactly as shown in image 2, letter-for-letter — do not invent alternate headline text, taglines, or captions as in-image text, and do not misspell it. If including text would crowd or distort the scene, omit text entirely rather than approximating it. Style: ${brand.visualStyle.imageGuidance}. Color palette: ${brand.visualStyle.colorPalette.join(", ")}. Square 1:1 aspect ratio for Instagram.`;
